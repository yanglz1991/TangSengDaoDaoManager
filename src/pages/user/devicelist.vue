          ...
          <el-form inline>
            <el-form-item class="mb-0 !mr-16px">
              <el-input
                ...
                placeholder="设备ID / 设备名 / 型号 / 用户uid / 昵称"
                clearable
                style="width: 320px"
...
import { userDeviceListGet, userDeviceBlacklistAddPost, userDeviceBlacklistRemovePost } from '@/api/user';
...
const onToggleBan = (row: any) => {
  ...
  const label = row.device_name || row.device_id;
  if (banned) {
    ElMessageBox.confirm(`确定要解禁设备「${label}」吗？`, '解禁设备', {
      ...
      .then(() => {
        userDeviceBlacklistRemovePost({ device_id: row.device_id })
          .then(() => {
    ...
    ElMessageBox.prompt(`请输入封禁设备「${label}」的原因（可选）`, '封禁设备', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      ...
      .then(({ value }: any) => {
        userDeviceBlacklistAddPost({ device_id: row.device_id, reason: value || '' })
          .then(() => {
